...
public class PaymentController {
    ...
    //  可以使用postman软件测试post请求
        //@RequestBody是用来客户端数据传到后端的，restTemplate，如果是后端自测，需要去掉@RequestBody注解
    ...
    public String Info_OK(@PathVariable("id") Integer id){
      String result = "线程池："+Thread.currentThread().getName()+" Info_OK,id：  "+id ;
      log.info("*****正常状态结果" + result);
      return result;
    }
    ...
    public String Info_Timeout(@PathVariable("id") Integer id){
        ...
        String result = "线程池："+Thread.currentThread().getName()+" TimeOut,id："+id+" 耗时(秒)"+timeNumber;
        log.info("****超时状态结果" + result);
        return result;
    }
}
